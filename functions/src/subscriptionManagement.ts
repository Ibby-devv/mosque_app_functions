...
// ============================================================================
// VERIFY TOKEN AND CREATE PORTAL SESSION
// ============================================================================

export const verifyManagementToken = onCall(
  {
    region: "australia-southeast1",
  ...
  async (request) => {
    ...
    try {
      ...
      // Get token
      const tokenDoc = await db.collection("managementTokens").doc(token).get();
      ...
      if (tokenData.expiresAt.toDate() < new Date()) {
        throw new Error("Link expired. Please request a new one.");
      }

      // Check if used
      if (tokenData.used) {
        throw new Error("Link already used. Please request a new one.");
      }
