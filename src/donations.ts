...
export const createPaymentIntent = onCall({
  ...
  try {
    ...
    const settings = await db.collection('donationSettings').doc('config').get();
...
export const cancelSubscription = onCall({
  ...
  try {
    ...
    const recurringRef = db.collection('recurringDonations').doc(subscriptionId);
...
export const generateReceiptNumber = async (): Promise<string> => {
  ...
  try {
    return await db.runTransaction(async (transaction) => {
      ...
      const receiptNumber = `RCP-${currentYear}-${newNumber.toString().padStart(5, '0')}`;
...
export const getDonationSettings = onCall({
  ...
  try {
    const settingsDoc = await db.collection('donationSettings').doc('config').get();
